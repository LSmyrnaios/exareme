...
do
    ...
    echo "1: Deploy the exareme kubernetes and start services."
    ...
    echo "7: (Re)Initialize the exareme kubernetes target machines' information (hosts.ini, vault.yaml)."
    echo "8: Add a new worker to the exareme kubernetes information files (hosts.ini, vault.yaml)."
	echo "9: Remove a worker from the exareme kubernetes inforation files (hosts.ini, vault.yaml)."
    echo -e "10:Exit.\n"
    ...
    while true
    do
		# 1: Deploy the exareme kubernetes and start services.
		if [[ "${answer1}" == "1" ]]; then
		...
		# 7: (Re)Initialize the exareme kubernetes target machines' information (hosts.ini, vault.yaml).
        elif [[ "${answer1}" == "7" ]]; then
            echo -e "\nYou chose to (re)initialize the target machines' information (hosts.ini, vault.yaml)..."
            . updateFiles.sh
            break
		
		# 8: Add a new worker to the exareme kubernetes information (hosts.ini, vault.yaml)."
        elif [[ "${answer1}" == "8" ]]; then
            echo -e "\nYou chose to add a new worker to the exareme kubernetes information."
        ...
        # 9: Remove a worker from the exareme kubernetes information files (hosts.ini, vault.yaml).
        elif [[ "${answer1}" == "9" ]]; then
		    echo -e "\nYou chose to remove a worker from the exareme kubernetes information."
