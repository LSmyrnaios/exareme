...
do
    if [[ "${answer}" == "y" ]]; then
        ...
        ansible_playbook_start=${ansible_playbook}"../Start-Exareme.yaml --skip-tags dashboard,portainerSecure"
        ${ansible_playbook_start}
        ...
        #If status code != 0 an error has occurred
        if [[ ${ansible_playbook_code} -ne 0 ]]; then
            echo "Playbook \"Start-Exareme.yaml --skip-tags dashboard,portainerSecure\" exited with error." >&2
            exit 1
        fi
        echo -e "\nExareme services are now running"
        break
