...
joinWorker () {
    ...
    if [[ ${ansible_playbook_code} -ne 0 ]]; then
        echo "Playbook \"Join-Workers.yaml\" exited with error. Try select 1 from main menu below to deploy the exareme kubernetes and start services.." >&2
        sleep 1
        break
    fi
    echo -e "\n${1} is now part of the kubernetes..\n"
    sleep 1
}
...
echo -e "\nWhat is the IP of the target worker you would like to add to the exareme kubernetes information files?"
read answer
...
if [[ ${tagExist} != "1" ]]; then
    ...
    #Join worker in kubernetes
    echo -e "\nAdd worker with IP: \"${workerIP}\" in an already initialized kubernetes.."
...
if [[ ${workerExist} != "1" ]]; then
    ...
    do
        if [[ ${answer} == y ]]; then
            # Add worker in hosts.ini, join worker in kubernetes, Start Exareme in worker
